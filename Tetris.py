...
class Env(tk.Tk):
    ...
    def _add_canvas(self): #블록모양 잡은거네 pos[0,0]은 첫번째 블록의 좌측상단 x 0,1 은 y 블록 4개그린거네 최종적으로 테트로미노 만드는부분이군
        pos = self.make_block()
    ...
    def _build_canvas(self):
        ...
        # 캔버스에 이미지 추가
        pos = self.make_block() #블록모양 잡은거네 pos[0,0]은 첫번째 블록의 좌측상단 x 0,1 은 y 블록 4개그린거네 최종적으로 테트로미노 만드는부분이군
